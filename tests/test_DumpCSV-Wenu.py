...
for file in os.listdir(ig_dir):
    ...
    if fnmatch.fnmatch(file, '*.ig'):
        ...
        for e in range(len(events)):
            ...
            charges = [ch for ch in event.GsfElectrons_V1.charge]
            pts = [pt for pt in event.GsfElectrons_V1.pt]
            etas = [eta for eta in event.GsfElectrons_V1.eta]
            phis = [phi for phi in event.GsfElectrons_V1.phi]
